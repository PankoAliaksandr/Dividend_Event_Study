...
pick_div_type_shiny <- function(stock_names,
  ...
  colnames(div_data_df) <- c("Ex-Date", "Dividend Type", "Stock Name")
  
  # Take into accound time window
  div_data_df <- div_data_df[div_data_df$`Ex-Date` >= start_date &
                               div_data_df$`Ex-Date` <= end_date, ]
  # reset index
  rownames(div_data_df) <- NULL
  
  available_div_types <- unique(div_data_df$`Dividend Type`)
  
  num_of_div_types <- length(available_div_types)
  
  if(num_of_div_types == 0){
    message("There were no dividends during selected period. Further research does not have any sense with these input parameters")
...
get_stock_abn_ret_window <- function(one_ExDiv_date, abnorm_ret_stock_xts, period){
  
  # one_ExDiv_date is supposed to be a data frame (checked earlier)
  # abnormal_ret_all_xts is time-series object (checked earlier)
  
  # get row with ExDiv date
  t0_ind <- which(index(abnorm_ret_stock_xts) == one_ExDiv_date)
...
get_stocks_abn_ret_window <- function(abnorm_ret_all_xts, ExDiv_dates, period){
  ...
  if(class(ExDiv_dates) != "data.frame"){
  ...
  for(i in 1:nrow(ExDiv_dates)){
    
    # pick ExDiv date
    one_ExDiv_date <- ExDiv_dates$`Ex-Date`[i]
    
    # pick the corresponding stock
    stock_name <- ExDiv_dates$`Stock Name`[i]
    ...
    if(sum(is.na(abnorm_ret_stock_xts)) == 0){
      ...
      l[j] <- get_stock_abn_ret_window(one_ExDiv_date = one_ExDiv_date,
      ...
      not_valid_stocks <- c(not_valid_stocks, stock_name)
    }
  }
  
  # Unite in one data frame all single stock's abn. ret.
  df <- data.frame(l)
  valid_stock_names <- ExDiv_dates$`Stock Name`[!(ExDiv_dates$`Stock Name` %in% not_valid_stocks)]
...
execute_ttest0 <- function(analized_df){
  ...
  for(i in 1:n){
    ...
    tryCatch(
      ...
      {
        # show original error message
        message("not enough observations for t.test")
        message(cond)
      }
    )
...
ui <- fluidPage(
   ...
   # Application title
   titlePanel(title = "Divident Event Study", windowTitle = "Divident Event Study"),
   ...
   sidebarLayout(
     sidebarPanel(
       ...
       selectInput("stocksi", label = "Stocks to use:", multiple = TRUE,
       ...
       selectInput("div_type", label = "Dividend types to use:", multiple = TRUE,
...
server <- function(input, output) {
  ...
  window_abn_ret_df <- reactive(get_stocks_abn_ret_window(abnorm_ret_all_xts = abnormal_returns(),
                                                          ExDiv_dates = index_data()$div_data_df,
